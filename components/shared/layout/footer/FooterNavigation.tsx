...
const FooterNavigation = () => {
  ...
  const items = [
    ...
    { alt:"دسته بندی محصولات", label: "دسته بندی ها", href: "/categories", imageUrl: "/images/icons/squares.svg", active: asPath === "/categories" },
  ...
  return (
    <>
      ...
      <div className="text-black dark:text-white fixed bottom-0 left-0 right-0 z-10">
        <nav className="min-h-20 flex rounded-t-xl bg-white dark:bg-[#192b39]">
          {items.map(item => (
            <Link
              prefetch={false}
              key={item.label}
            ...
            >
              <div className="relative w-fit">
...

                {
                  ...
                  <span className="absolute -top-1 -right-2 bg-red-500 text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
